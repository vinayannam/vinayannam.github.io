...
function Circle(x, y, r, i, randomMe, randomYou) {
    ...
    this.draw = function () {
        ...
        if(i == randomMe){
            c.font = "20px Arial";
            c.fillText("Me", this.x+(this.r), this.y+(this.r));
        }
        if(i == randomYou){
            c.font = "20px Arial";
            c.fillText("You", this.x+(this.r), this.y+(this.r));
        }
    }
}
...
function init() {
    const randomMe = Math.floor(Math.random() * prop.count)
    const randomYou = Math.floor(Math.random() * prop.count)
    for (let i = 0; i < prop.count; i++) {
        const randomX = Math.random() * canvas.width
        const randomY = Math.random() * canvas.height
        const randomR = Math.random() * 8
        circleArray.push(
            new Circle(randomX, randomY, randomR, i, randomMe, randomYou)
        )
    }
}
